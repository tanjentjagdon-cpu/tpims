...
// Standalone parser logic (copied from src/lib/shopee-parser.ts with the fix)
...
async function run() {
    console.log('Importing Shopee Order Mnanually...');

    // 1. Get a user_id from existing orders
    ...
    if (userError || !userData || userData.length === 0) {
        console.error('Could not find a user_id in shopee_orders. Please check if there are any orders.');
        return;
    }
    ...
    const rows = parsed.items.map((item, index) => {
        ...
        return {
            ...
            total_payment: isFirst ? parsed.totals.total_payment : 0, // Using total_payment for first item
    ...
    console.log(`Inserting ${rows.length} items for Order ${parsed.order_id}...`);

    // 4. Delete existing and Insert
    ...
    if (insertError) {
        ...
        console.log('Successfully imported order!');
    }
}
